...
class AnimatedGif
{
	...
	* @param array    $image_delays             The delays associated with the
	*                                           source images
	* @param int      $number_of_loops          The number of times to loop
	* @param int|null $transparent_colour_red
	* @param int|null $transparent_colour_green
	* @param int|null $transparent_colour_blue
	*/
	public function __construct(
		...
		*/
		...
		$this->setTransparentColour($transparent_colour_red, $transparent_colour_green, $transparent_colour_blue);
		try {
	...
	* @param int $blue
	*/
	private function setTransparentColour(int $red, int $green, int $blue): void
	{
	...
	* @throws Exception
	*/
	private function bufferImages(array $source_images): void
	{
		for ($i = 0; $i < count($source_images); $i++) {
			$this->buffer[] = $source_images[$i];
			if (substr($this->buffer[$i], 0, 6) != "GIF87a" &&
				substr($this->buffer[$i], 0, 6) != "GIF89a"
			) {
	...
	* @return int
	*/
	private function blockCompare($GlobalBlock, $LocalBlock, $Len): int
	{
		for ($i = 0; $i < $Len; $i++) {
